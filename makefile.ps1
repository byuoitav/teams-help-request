...
function DockerFunc {   #can not just be docker because it creates an infinite loop
    ...
    if ($COMMIT_HASH -eq $TAG) {
        ...
        Invoke-Expression "docker build -f dockerfile --build-arg NAME=$NAME -t $DOCKER_PKG-dev:$COMMIT_HASH dist"
    } elseif ($TAG -match $DEV_TAG_REGEX) {
        Write-Output "Building dev container with tag $TAG"
        Invoke-Expression "docker build -f dockerfile --build-arg NAME=$NAME -t $DOCKER_PKG-dev:$TAG dist"
    } elseif ($TAG -match $PRD_TAG_REGEX) {
        Write-Output "Building prd container with tag $TAG"
        Invoke-Expression "docker build -f dockerfile --build-arg NAME=$NAME -t ${DOCKER_PKG}:$TAG dist"
    } else {
...
function Deploy {
    ...
    if ($COMMIT_HASH -eq $TAG) {
        ...
        Invoke-Expression "docker push $DOCKER_PKG-dev:$COMMIT_HASH"
    } elseif ($TAG -match $DEV_TAG_REGEX) {
        Write-Output "Pushing dev container with tag $TAG"
        Invoke-Expression "docker push $DOCKER_PKG-dev:$TAG"
    } elseif ($TAG -match $PRD_TAG_REGEX) {
        Write-Output "Pushing prd container with tag $TAG"
        Invoke-Expression "docker push ${DOCKER_PKG}:$TAG"
    } else {
